...
@Injectable()
export class UriVersioningService implements IVersioningService {
  ...
  getVersion(request: Record<string, unknown>): string {
    const versionParam = this.config.parameterName ?? 'version';
    const version = request[versionParam] as string | undefined;
