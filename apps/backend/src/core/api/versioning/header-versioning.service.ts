...
@Injectable()
export class HeaderVersioningService implements IVersioningService {
  readonly type: VersioningType = 'header';
  ...
  getVersion(request: Record<string, unknown>): string {
    const headerName = (this.config.headerName ?? 'x-api-version').toLowerCase();
    const version = request[headerName] as string | undefined;
