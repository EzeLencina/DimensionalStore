import type { CreateCollectionCommand } from '../../commands/collection';

export class CollectionValidator {
  static validateCreate(command: CreateCollectionCommand): string[] {
    const errors: string[] = [];
    ...
    if (command.startAt && command.endAt && new Date(command.startAt) >= new Date(command.endAt)) {
