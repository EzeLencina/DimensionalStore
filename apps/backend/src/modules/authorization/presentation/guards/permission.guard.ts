...
export class PermissionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!user?.userId) {
      this.logger.warn({ event: 'authz.permission_guard.no_user' }, 'No authenticated user');
      throw new ForbiddenException('Authentication required');
    }

    for (const { resource, action } of required) {
      const result = await this.authzService.checkAccess(user.userId, resource, action);
      if (!result.granted) {
        this.logger.warn(
          { event: 'authz.permission_guard.denied', userId: user.userId, resource, action, reason: result.reason },
          'Permission denied',
        );
    ...
    this.logger.info(
      { event: 'authz.permission_guard.granted', userId: user.userId, required },
      'Permission granted',
    );
    return true;
  }
